...
for i, ax in enumerate(axs.flat):
    ...
    ax.set_yticks([])
    ax.set_aspect('equal')
    ax.imshow(faces.images[k], cmap='bone', interpolation='none')
    print(k)
...
ax.imshow(pca.mean_.reshape((64, 64)), cmap=plt.cm.bone)
fig.tight_layout(pad=0.1)
...
for i, ax in enumerate(axs.flat):
    ...
    ax.imshow(im, cmap='bone', interpolation='none')
...
def recon_frame(idx, n=0):
    ...
    ax[0].imshow(faces.images[idx], cmap='bone', interpolation='none')
    ...
    ax[1].set_xlim(0, 64)
    ax[1].set_ylim(64, 0)
    ax[1].imshow(reconstruct(idx, n), cmap='bone', interpolation='none')
    fig.tight_layout(pad=0.1)
    plt.close(fig)
    return fig, ax
